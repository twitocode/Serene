...
public class StepOneRequestValidator : AbstractValidator<StepOneRequest>
{
    public StepOneRequestValidator()
    {
        RuleFor(x => x.Name)
            ...
            .Matches(@"^[a-zA-Z\s-]+$")
            .WithMessage("Username can only contain letters, spaces, hyphens");
    }
}
...
public class StepTwoRequestValidator : AbstractValidator<StepTwoRequest>
{
    public StepTwoRequestValidator()
    {
        RuleFor(x => x.DateOfBirth)
            .NotEmpty()
            .WithMessage("Date of birth was not provided")
            .Must(BeAtLeast13YearsOld)
            .WithMessage("Must be 13 years or older")
        ...
        RuleFor(x => x.Gender)
            ...
            .Matches("^(Male|Female|Non-Binary|Prefer not to say)$")
            .WithMessage("Please select a valid gender");

        RuleFor(x => x.Pronouns)
            .NotEmpty()
            .WithMessage("Pronouns was not provided")
            .Matches(
                @"^(She\s?[\/\-]\s?Her|He\s?[\/\-]\s?Him|They\s?[\/\-]\s?Them|Prefer not to say)$"
            )
            .WithMessage("Invalid pronouns");
    }
...
public class StepThreeRequestValidator : AbstractValidator<StepThreeRequest>
{
    public StepThreeRequestValidator()
    {
        RuleFor(x => x.CountryCode)
            .NotEmpty()
            .WithMessage("Country Code was not provided")
            .MinimumLength(2)
            .WithMessage("Provided an invalid country code");
    }
}
...
public class StepFourRequestValidator : AbstractValidator<StepFourRequest>
{
    public StepFourRequestValidator()
    {
        RuleFor(x => x.Name)
            .NotEmpty()
            .WithMessage("School's name was not provided")
            .MinimumLength(2)
            .WithMessage("School's name must be at least 2 characters")
            .MaximumLength(100)
            .WithMessage("School's name must be less than 100 characters");

        RuleFor(x => x.CountryCode)
            .NotEmpty()
            .WithMessage("School's country code was not provided");

        RuleFor(x => x.City).NotEmpty().WithMessage("School's city was not provided");

        RuleFor(x => x.RegionCode)
            .NotEmpty()
            .WithMessage("School's region code was not provided")
            .Length(2)
            .WithMessage("Region code must be exactly 2 characters");
    }
}
