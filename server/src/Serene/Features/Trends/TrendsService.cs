...
public class TrendsService : ITrendsService
{
    ...
    public async Task<TrendsResponse> GetTrendsAsync(string userId, int year)
    {
        ...
        var thisYearCheckins = await _context.Checkins
            .Where(c => c.UserId == userId && c.DateCompleted >= startOfYear && c.DateCompleted < endOfYear)
            .ToListAsync();

        var prevYearCheckins = await _context.Checkins
            .Where(c => c.UserId == userId && c.DateCompleted >= startOfPrevYear && c.DateCompleted < startOfYear)
            .ToListAsync();
        ...
        var moodCalendar = Enumerable.Range(1, 12)
            .Select(month =>
            {
                ...
                return new MoodCalendarMonth
                {
                    ...
                    Days = Enumerable.Range(1, daysInMonth)
                        .Select(day =>
                        {
                            var dayStart = new LocalDate(year, month, day).AtStartOfDayInZone(zone).ToInstant();
                            var dayEnd = new LocalDate(year, month, day).PlusDays(1).AtStartOfDayInZone(zone).ToInstant();
                            var dayCheckin = thisYearCheckins
                                .FirstOrDefault(c => c.DateCompleted >= dayStart && c.DateCompleted < dayEnd);
        ...
        var topActivities = thisYearCheckins
            ...
            .Take(5)
            .Select((a, _) => new TopActivityItem
            {
                Activity = a.Activity,
                Count = a.Count,
                Percentage = thisYearCheckins.Count > 0 
                    ? Math.Round((double)a.Count / thisYearCheckins.Count * 100, 1) 
                    : 0
            })
            .ToList();

        var energyLevels = Enumerable.Range(1, 12)
            .Select(month =>
            {
                var monthStart = new LocalDate(year, month, 1).AtStartOfDayInZone(zone).ToInstant();
                var monthEnd = new LocalDate(year, month, 1).PlusMonths(1).AtStartOfDayInZone(zone).ToInstant();
                ...
                return new EnergyLevelPoint
                {
                    Month = month,
                    MonthName = MonthNames[month - 1],
                    AverageLevel = monthCheckins.Count > 0 
