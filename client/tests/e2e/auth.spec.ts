...
test.describe("Authentication Flows", () => {
	test("should show error on non-existent email", async ({ page }) => {
		...
		await page.getByPlaceholder(/m@example.com/i).fill("nonexistent@example.com");
	...
	test("should transition to password step when email exists", async ({ page }) => {
		await page.goto("/login");
	...
	test("should handle successful login and redirect", async ({ page, context }) => {
		await page.goto("/login");
		await page.getByPlaceholder(/m@example.com/i).fill("test@example.com");
		await page.getByRole("button", { name: /continue/i }).click();
