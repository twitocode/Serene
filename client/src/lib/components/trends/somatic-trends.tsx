...
import { SomaticData } from "@/lib/hooks/queries/use-trends";
...
export function SomaticTrends({ data }: SomaticTrendsProps) {
  ...
  const mockSomaticState = hasData 
    ? Object.keys(partCounts).reduce((acc, part) => {
  ...
  return (
    <TrendCard title="Physical Sensations" subtitle="Where you've felt tension or discomfort most often">
      <div className="flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="flex-1 space-y-4">
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Top Sensations</h4>
            <div className="flex flex-wrap gap-2">
              {topSensations.length > 0 ? (
                topSensations.map((s, i) => (
                  <Badge key={i} variant="secondary" className="bg-zinc-100 text-zinc-800 hover:bg-zinc-200 border-none">
                    {s.sensation} <span className="ml-1 opacity-50">{s.count}</span>
                  </Badge>
                ))
              ) : (
                <span className="text-sm text-muted-foreground">No sensation data yet.</span>
              )}
            </div>
          </div>
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Most Affected Areas</h4>
                 ...
                 Object.entries(partCounts)
                  ...
                  .map(([part, count], i) => (
                    <div key={i} className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-zinc-400 font-medium">{count} reports</span>
                    </div>
                  ))
               ) : (
                 <span className="text-sm text-muted-foreground">Keep checking in to see patterns.</span>
               )}
            </div>
          </div>
        </div>
