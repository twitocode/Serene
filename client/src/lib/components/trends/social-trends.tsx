...
import { CommunityStats } from "@/lib/hooks/queries/use-trends";
...
export function SocialTrends({ data }: SocialTrendsProps) {
  ...
  return (
    <TrendCard title="Community & Growth" subtitle="Your engagement within the Serene community">
      <div className="space-y-6">
        <div className="grid grid-cols-3 gap-4">
          <div className="flex flex-col items-center text-center p-3 rounded-2xl bg-zinc-50 border border-zinc-100">
            ...
            <span className="text-[10px] text-muted-foreground uppercase">Answers</span>
          </div>
          <div className="flex flex-col items-center text-center p-3 rounded-2xl bg-zinc-50 border border-zinc-100">
          ...
          <div className="flex flex-col items-center text-center p-3 rounded-2xl bg-zinc-50 border border-zinc-100">
            ...
            <span className="text-[10px] text-muted-foreground uppercase">Support</span>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {interests && interests.length > 0 ? (
              interests.map((interest, i) => (
                <Badge key={i} variant="outline" className="rounded-full border-zinc-200 text-zinc-600 font-normal py-1 px-3">
                  {interest}
                </Badge>
              ))
            ) : (
