...
function BarChart({
	...
	const yScale = scaleLinear<number>({
		...
		domain: [
			0,
			Math.max(...data.map((d) => Math.max(d.thisYear, d.previousYear)), 1),
		],
	});
...
export function MoodBreakdownChart({ data }: MoodBreakdownChartProps) {
	...
	const allMoodLabels = Array.from(
		new Set([
	...
	const chartData = allMoodLabels.map((label) => ({
		mood: label,
		thisYear:
			data?.thisYear.find((m) => m.moodLabel.toLowerCase() === label)?.count ||
			0,
		previousYear:
			data?.previousYear.find((m) => m.moodLabel.toLowerCase() === label)
				?.count || 0,
	}));
