...
import { ActivityImpactItem } from "@/lib/hooks/queries/use-trends";

interface ActivityImpactChartProps {
  data?: ActivityImpactItem[];
}

function ImpactChart({ data, width, height }: { data: ActivityImpactItem[], width: number, height: number }) {
...
export function ActivityImpactChart({ data }: ActivityImpactChartProps) {
  ...
  return (
    <TrendCard title="Mood Boosters" subtitle="Activities that resulted in the highest mood increase">
      {hasData ? (
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            {displayData.map((d, i) => (
              <div key={i} className="flex items-center gap-2 overflow-hidden">
                ...
                <span className="text-[10px] text-muted-foreground truncate uppercase tracking-tight">{d.activity}</span>
              </div>
            ))}
          </div>
        </div>
