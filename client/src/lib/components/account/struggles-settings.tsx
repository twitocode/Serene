...
import { useUpdateProfileMutation, useUserQuery } from "@/lib/hooks/queries/use-user";
...
export function StrugglesSettings() {
  ...
  const strugglesString = user?.profile?.struggles?.join(",") ?? "";

  useEffect(() => {
    if (user?.profile?.struggles) {
      setSelected(user.profile.struggles);
    }
  }, [strugglesString, user?.profile?.struggles]);
